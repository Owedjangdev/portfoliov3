export const worksStyles = {
  ...
  label: 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-[0.2em] mb-4 border border-blue-100 dark:border-blue-500/20',
  title: 'text-3xl sm:text-4xl md:text-5xl font-black font-[Clash_Display] text-slate-900 dark:text-white leading-tight break-words',
  titleAccent: 'font-[Clash_Display] bg-gradient-to-r from-blue-500 to-cyan-400 bg-clip-text text-transparent ml-2',

  // Zone de défilement
  scrollWrapper: 'relative w-full overflow-hidden py-10', 
  track: 'flex gap-6 md:gap-8',

  // Carte style Browser
  card: 'flex flex-col min-w-[300px] md:min-w-[420px] bg-white dark:bg-[#16161F] rounded-lg overflow-hidden border border-slate-200 dark:border-white/5 shadow-sm hover:shadow-xl transition-all duration-500',
  ...
  overlay: 'absolute inset-0 bg-blue-600/10 opacity-0 hover:opacity-100 transition-opacity duration-500 flex items-center justify-center backdrop-blur-[2px] cursor-pointer',
  ...
  button: 'group flex items-center gap-3 bg-slate-900 dark:bg-white text-white dark:text-black px-8 py-4 rounded-lg font-bold text-sm transition-all hover:scale-105 active:scale-95 shadow-lg'
}
